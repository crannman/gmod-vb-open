...
function ENT:DrawTranslucent()
  ...
  if alpha > 0  then
    ...
    local title = "Micro-onde"
    local text = string.format("Interargissez avec le %s pour préparer une recette", title:lower())
    local scale = 0.05
    -- start 3d2d context
    cam.Start3D2D(pos + Vector(0, 0, height + 15), Angle(0, ang.y, 90), scale)
